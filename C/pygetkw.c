...
#define ASSERT_VAL(val)                                                             \
  if (val == NULL) {                                                                \
    Py_DECREF(self->kw);                                                            \
    MSG_ERROR2_0("Value error:", path);                                             \
    return 0;                                                                       \
  }
#define ASSERT_MEM(mem)                                                             \
  if (mem == NULL) {                                                                \
    Py_DECREF(self->kw);                                                            \
...
static int except_error(PyObject * obj) {
  ...
  if (verbose) {
    ...
    if (b != NULL)
      PyObject_Print(b, stderr, Py_PRINT_RAW);
    fprintf(stderr, "\n");
  }
...
static int get_kw_obj(Getkw_t * self, const char * path, const char * type) {
  ...
  if (!check_kw_type(obj, type)) {
    Py_DECREF(obj);
    if (verbose)
      MSG_ERROR2_0("Keyword is not of type", type);
    return 0;
  }
