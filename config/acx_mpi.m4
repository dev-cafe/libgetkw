...
LIBS="$LIBS $MPILIBS"
AC_LANG_CASE(
    [C], [AC_CHECK_FUNC(MPI_Init, [acx_mpilibs_ok=yes])],
	[C++], [AC_CHECK_FUNC(MPI_Init, [acx_mpilibs_ok=yes])],
	[Fortran 77], [AC_MSG_CHECKING([for MPI_Init])
		AC_LINK_IFELSE([AC_LANG_PROGRAM([],[      call MPI_Init])],
		[acx_mpilibs_ok=yes; AC_MSG_RESULT(yes)], [AC_MSG_RESULT(no)])],
	[Fortran], [AC_MSG_CHECKING([for MPI_Init])
		AC_LINK_IFELSE([AC_LANG_PROGRAM([],[      call MPI_Init])],
		[acx_mpilibs_ok=yes; AC_MSG_RESULT(yes)], [AC_MSG_RESULT(no)])])
LIBS="$acx_save_LIBS"

if test "$acx_mpilibs_ok" = "no"; then
	AC_LANG_CASE([Fortran 77], [
		if test "$acx_mpilibs_ok" = "no"; then
			AC_CHECK_LIB(fmpi, MPI_Init, 
				[acx_mpilibs_ok=yes; MPILIBS="-lfmpi"])
		fi
		if test "$acx_mpilibs_ok" = "no"; then
			AC_CHECK_LIB(fmpich, MPI_Init, 
				[acx_mpilibs_ok=yes; MPILIBS="-lfmpich"])
		fi
	],
	[Fortran], [
		if test "$acx_mpilibs_ok" = "no"; then
		...
		if test "$acx_mpilibs_ok" = "no"; then
			AC_CHECK_LIB(mpichf90, MPI_Init, 
				[acx_mpilibs_ok=yes; MPILIBS="-lmpichf90"])
		fi
	])
	if test "$acx_mpilibs_ok" = "no"; then
		AC_CHECK_LIB(mpi, MPI_Init, 
				[acx_mpilibs_ok=yes; MPILIBS="-lmpi"])
	fi
	if test "$acx_mpilibs_ok" = "no"; then
		AC_CHECK_LIB(mpich, MPI_Init, 
				[acx_mpilibs_ok=yes; MPILIBS="-lmpich"])
	fi
fi
...
if test "$acx_mpilibs_ok" = "yes"; then
AC_LANG_CASE(
[C], [
	AC_MSG_CHECKING([for mpi.h])
	AC_TRY_COMPILE([#include <mpi.h>],[],
		[acx_mpi_ok=yes; AC_MSG_RESULT(yes)], [AC_MSG_RESULT(no)])],
[C++], [
	AC_MSG_CHECKING([for mpi.h])
	AC_TRY_COMPILE([#include <mpi.h>],[],
		[acx_mpi_ok=yes; AC_MSG_RESULT(yes)], [AC_MSG_RESULT(no)])],
...
dnl make sure MPILIBS is defined
